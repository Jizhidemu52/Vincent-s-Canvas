...
test("moving a rectangle keeps its dimensions and undo restores its original position", () => {
    const ops = [rectangle, editorRectangleMove([rectangle], 0, { x: 100, y: 50 })];
    ...
    expect(editorRectangles([...ops, editorRectangleMove(ops, 0, { x: -10, y: 0 })], size)[0].x).toBe(110);
});
test("dragging after crop and rotation uses the current displayed coordinates", () => {
    ...
    const moved = editorRectangles([...ops, editorRectangleMove(ops, 0, { x: 10, y: 20 })], size)[0];
