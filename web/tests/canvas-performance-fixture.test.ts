...
const connections = Array.from({ length: 1000 }, (_, index) => ({
    ...
    to: nodes[(index + (index % 2 === 0 ? 1 : 25)) % nodes.length],
}));
...
describe("canvas performance fixture", () => {
    test("culls a 500-node and 1000-connection project to the current viewport", () => {
        ...
        const visibleConnections = connections.filter((connection) => connectionIntersectsCanvasBounds(connection.from, connection.to, bounds));
