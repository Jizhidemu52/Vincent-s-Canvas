...
import { getVideoModelParameterSpec, normalizeVideoModelConfig, videoAspectRatioFollowsImage, type VideoModelParameterKey } from "@/lib/video-model-parameters";
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", referenceCount, imageMode }: VideoSettingsPanelProps) {
    ...
    const ratioFollowsImage = videoAspectRatioFollowsImage(config, referenceCount, imageMode);
            ...
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                {!spec ? <p className="text-sm leading-6" style={{ color: theme.node.muted }}>当前模型尚未提供参数规格，请先选择已配置的视频模型。</p> : <>
                    ...
                    <SettingGroup title="画面比例" color={theme.node.muted}>
                        {ratioFollowsImage ? <div className="rounded-xl border px-3 py-2.5 text-sm" style={{ borderColor: theme.node.stroke }}>跟随输入图片，无需设置比例</div> : <div className="grid grid-cols-3 gap-2.5">
                    ...
                    <SettingGroup title="时长" color={theme.node.muted}>
                        <select aria-label="视频时长" className="h-9 w-full cursor-pointer rounded-xl border px-3 text-sm outline-none" style={{ background: theme.node.fill, borderColor: theme.node.stroke, color: theme.node.text }} value={normalized.videoSeconds} onChange={(event) => onConfigChange("videoSeconds", event.target.value)}>
                    ...
                    <div className="space-y-1 text-[11px] leading-4" style={{ color: theme.node.muted }}><p>{spec.imageInputHint}</p>{spec.audioHint ? <p>{spec.audioHint}</p> : null}</div>
                </>}
            </div>
        </ImageSettingsTheme>
    );
