...
export function AuthGate({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
    ...
    if (!user) {
        if (deploymentFeatures.oaLoginEnabled) return <OaEntryNotice />;
    ...
    if (deploymentFeatures.oaLoginEnabled && admin) return <Navigate to="/" replace />;
    if (!deploymentFeatures.oaLoginEnabled && user.mustChangePassword && location.pathname !== "/change-password") return <Navigate to="/change-password" replace />;
