...
import { createPriceDraft, createServerModel, listPriceVersions, listServerModels, listServerProviders, markPriceTesting, publishPrice, updateServerModel, type PriceVersion, type ServerModel, type ServerProvider } from "@/services/api/model-configuration";

const operationOptions = [
    ["image_generation", "生成一张图"], ["upscale", "放大图片"], ["remove_background", "去背景"],
    ["inpaint", "局部编辑"], ["batch_image", "批量处理每张图"], ["seamless_stitch", "无缝拼接"],
].map(([value, label]) => ({ value, label }));
const capabilityOptions = ["generate", "edit", "upscale", "remove_background", "batch"].map((value) => ({ value, label: value }));

type PriceValues = { operationType: string; label: string; credits: number; rmbCost: number };
type ModelValues = Omit<ServerModel, "id" | "providerName">;
...
export function ModelPricingPanel({ mode }: { mode: "prices" | "models" }) {
    ...
    const refresh = async () => {
        setLoading(true);
        try {
            const [providerResult, priceResult, modelResult] = await Promise.all([listServerProviders(), listPriceVersions(), listServerModels()]);
            setProviders(providerResult.providers); setPrices(priceResult.prices); setModels(modelResult.models);
    ...
    const editModel = (model: ServerModel) => { setEditingModelId(model.id); modelForm.setFieldsValue({ providerId: model.providerId, name: model.name, modelId: model.modelId, capabilities: model.capabilities, creditCost: model.creditCost, rmbCost: model.rmbCost, concurrencyLimit: model.concurrencyLimit, enabled: model.enabled }); };
    ...
    return (
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
            <Table rowKey="id" size="small" loading={loading} dataSource={models} columns={[
                { title: "模型名称", dataIndex: "name" }, { title: "模型 ID", dataIndex: "modelId" }, { title: "Provider", dataIndex: "providerName" },
