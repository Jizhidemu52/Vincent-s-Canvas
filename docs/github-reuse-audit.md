...
- Basketikun-inspired asset dock reuse flow with saved asset insertion, project-folder grouping, tag filtering, and text search.
...
3. Expand basketikun-style asset and prompt organization with richer card metadata and deeper image storage boundaries now that asset folders/search/filtering plus prompt category/favorite filtering and favorite write-back exist.
