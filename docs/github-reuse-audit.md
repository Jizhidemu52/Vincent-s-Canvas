...
- Basketikun-inspired asset dock reuse flow with saved asset insertion, tag filtering, and text search.
...
| Asset persistence/search and image storage boundaries | `basketikun/infinite-canvas` | `use-asset-store.ts`, `image-storage.ts`, `file-storage.ts`, `app-sync.ts` | Adapted asset tag filtering and text search in the canvas dock on 2026-06-28 using existing backend-owned `LibraryAsset` metadata; folders and image storage boundaries remain later. |
...
3. Expand basketikun-style asset and prompt organization with asset folders, prompt categories, favorites, and richer card metadata now that asset search/filtering and prompt detail handoff exist.
