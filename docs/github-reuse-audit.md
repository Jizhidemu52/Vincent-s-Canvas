...
- Basketikun-inspired prompt library service boundary with backend-owned designer prompt presets, attribution, category/favorite filtering, prompt detail cards, and canvas note handoff.
...
| Prompt library UI and prompt detail/selection behavior | `basketikun/infinite-canvas` | `web/src/components/prompts/*`, `web/src/services/api/prompts.ts` | Adapted backend prompt API boundary, designer attribution, category/favorite filters, detail preview cards, direct prompt insertion, and prompt-to-canvas-note handoff on 2026-06-28; favorite write-back and richer prompt metadata remain later. |
...
3. Expand basketikun-style asset and prompt organization with favorite write-back, richer card metadata, and deeper image storage boundaries now that asset folders/search/filtering and prompt category/favorite filtering exist.
