...
- Image nodes now expose real four-corner resize handles, so designers can drag any selected-image corner to resize instead of hunting for a single hidden handle.
- Basketikun-inspired asset dock reuse flow with saved asset insertion, project-folder grouping, tag filtering, text search, and folder/tag metadata write-back.
- Lightweight workflow modules and connections.
- Typed first-pass workflow module registry for `Generate / Edit / Upscale / Remove BG / Batch / Upload Reference / Final`, including backend API path mapping, input/output port definitions, selected-node inspector summaries, strict connection compatibility, compatible module picker filtering with target port labels, drag-time compatible/incompatible connection highlighting, port-level input/output target feedback, true port-specific drop connections, connected text-port prompt execution, connected image/result-port reference execution, connected config-port output-count execution, typed batch settings handoff for concurrency/failure policy, typed edit mask config handoff, typed provider request settings for size/quality/preset/RunningHub AI app fields/ComfyUI workflow JSON, and no-charge Final handoff assets.
- Batch import and processing.
- T8-inspired batch queue status summaries, failed-item retry batches, cancellation, pause/resume for long-running backend batches, merge-safe retry/resume results, backend stop-on-failure execution, skipped-item queue status, batch settings propagation into generation requests, and tested concurrency-limited batch submission.
- Basketikun-inspired prompt library service boundary with backend-owned designer prompt presets, attribution, category/favorite filtering, favorite write-back, prompt detail cards, and canvas note handoff.
- Backend-hosted provider registry, profile/history, admin credit adjustment, provider health, provider payload mapping, RunningHub AI app `webappId/nodeInfoList/taskType/instanceType` payloads, server-side `apiKey` body injection, POST output polling, live-ready execution boundary, HTTP live/mock routing, provider progress snapshots, node-level provider progress metadata, selected-node provider progress display, provider error normalization, multi-provider success/partial-success output normalization, and persistence.
- Admin credit limits, model default pricing controls, designer-visible pricing rules, and operation-level pricing overrides for generate/edit/upscale/remove-bg accounting.
...
| Prompt library UI and prompt detail/selection behavior | `basketikun/infinite-canvas` | `web/src/components/prompts/*`, `web/src/services/api/prompts.ts` | Adapted backend prompt API boundary, designer attribution, category/favorite filters, favorite write-back, detail preview cards, direct prompt insertion, and prompt-to-canvas-note handoff on 2026-06-28; richer prompt metadata remains later. |
| Asset persistence/search and image storage boundaries | `basketikun/infinite-canvas` | `use-asset-store.ts`, `image-storage.ts`, `file-storage.ts`, `app-sync.ts` | Adapted asset project-folder grouping, folder filtering, tag filtering, text search, and folder/tag metadata write-back in the canvas dock on 2026-06-28 using existing backend-owned `LibraryAsset` metadata. Added backend-owned reusable image asset intake on 2026-06-29: `POST /api/assets` accepts image data URLs, stores bytes in server state/SQLite `asset_blobs`, returns `/api/assets/:id/content`, keeps base64 out of asset lists/workspace snapshots, isolates content by designer account, and the canvas `Save asset` action now uses this hosted path for uploaded data-URL image nodes with local fallback for non-hostable sources. |
...
| RunningHub/ComfyUI provider adapters | `T8mars/T8-penguin-canvas`, `hero8152/Infinite-Canvas` | T8 backend provider files and `rh.md`; hero settings/workflow JSON | Adapted first-pass OpenAI/NanoBanana image payload, RunningHub/ComfyUI workflow payload mapping, live HTTP submission, polling, output normalization, HTTP generation routing by provider mode, provider progress snapshots, node-level provider progress metadata and context display, provider-specific error normalization, NanoBanana base64 outputs, nested workflow asset outputs, and partial-success item error reporting on 2026-06-28. Added T8 `rh.md`-aligned RunningHub AI app fixtures on 2026-06-28: provider settings now support `webappId`, `nodeInfoList`, `taskType`, `instanceType`, ComfyUI workflow JSON, server-side body `apiKey` injection, RH code/taskId status normalization, `fileUrl` output normalization, and POST `/task/openapi/outputs` polling without exposing secrets in admin payloads; next step is real async worker persistence. |
...
| History bulk management | `hero8152/Infinite-Canvas` | `static/js/history-bulk-manager.js` | Adapted filtered admin team history JSON/CSV export, selected-row CSV export, selected-row soft archive with audit logging, archived-review filtering, selected-row restore, selected-row permanent delete for archived records, designer-side project history filtering, original/result thumbnail review, and output-thumbnail deep links on 2026-06-28/29 so managers and designers can trace generation records by account/project, compare source references with results, hide or restore noisy team review rows without changing the credit ledger, permanently clean archived rows after finance export, and reopen generated nodes for the next edit round. |
...
2. Add remaining ComfyUI workflow fixtures by reconciling hero workflow JSON/settings fields with real uploaded workflow examples.
3. Expand basketikun-style asset and prompt organization with richer preview/card metadata, retention controls, and external object-storage migration now that asset folders/search/filtering/write-back, backend-owned asset content URLs, plus prompt category/favorite filtering and favorite write-back exist.
4. Extend admin/team history retention policy controls after real provider output storage is stable.
