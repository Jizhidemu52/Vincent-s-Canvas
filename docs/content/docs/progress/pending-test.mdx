...
- 后台管理：新增 `/admin` 后台页、本地账号额度中心、积分/价格规则、模型元数据配置、历史记录导出、项目素材监管、批量任务列表和审计日志；生图工作台接入后台积分预估与成功后扣费，仍需人工验证多设计师切换、额度不足拦截、导出 CSV 和后续服务端 API Key 管理方案。
...
- 提示词库：移除 Next.js route 抓取和服务端内存缓存，改为浏览器前端直连 远程原始文件并缓存到 IndexedDB。
