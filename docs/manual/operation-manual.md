...
![无缝拼接参数界面](../assets/github/seamless-stitch-parameters.png)

![无缝拼接实操结果](../assets/github/seamless-stitch-demo.png)

![真实内部接口结果](../assets/github/seamless-stitch-real-result.png)

适合场景：

- 面料、印花、墙纸或纹理图需要做连续平铺。
- 想快速生成可连续平铺的花型或纹理结果。
- 可按小花、大花预设，或用高级参数适配不同花型。

操作步骤：

1. 点左侧 `无缝拼接`。
2. 在 `参考图` 区域粘贴图片、上传图片，或从素材库选择图片。
3. 选择 `小花` 或 `大花` 预设。小花建议 `100 / 100 / 50 / 0.5`；大花建议 `200 / 200 / 100 / 1`。
4. 需要更细控制时，选择 `自定义`，填写切割宽度、重绘宽度、羽化、重绘强度和步数。
5. 确认提交按钮旁显示当前价格，默认是 `2 积分`。
6. 点 `开始无缝拼接`。
7. 右侧出现结果后，可以下载图片，结果也会保存到素材库和历史记录。

本地演示服务同样会生成可见的平铺结果并扣除 2 积分，方便在没有真实 App Key 时验证上传、任务状态、结果展示和余额同步。正式环境使用管理员绑定的内部 AI 工作流。
...
6. 填当前剩余额度和每月固定额度。新账号通常把两者填成相同数值。
7. 点 `开通账号`。
...
![调整本月临时额度](../assets/github/monthly-credit-temporary-action.png)

1. 找到右侧 `调整本月临时额度`。
2. 选择设计师。
3. 在 `仅本月有效的积分变化` 输入正数，例如 `500`。
4. 在原因里写说明，例如 `项目补充额度`。
5. 点 `保存调整`。
6. 这 500 只影响当前月；下月 1 日不会加入固定额度。
...
![设置每月固定额度](../assets/github/monthly-credit-fixed-action.png)

1. 找到 `设置每月固定额度`。
2. 选择设计师。
3. 输入每月 1 日重置后的积分，例如 `1000`。
4. 点 `保存每月固定额度`。

自然月规则（时区为 `Asia/Shanghai`）：

- 固定额度 1000、本月临时增加 500：固定额度仍是 1000，本月余额增加 500。
- 下月 1 日首次登录、管理员查看账号或提交任务时，服务端把余额重置为 1000。
- 上月没用完的余额和临时额度都不结转。
- 月度重置、临时增加和临时扣减都会写入额度账本，重复请求不会重复加分。
- 上月冻结的任务跨月失败时，不会把过期额度退入新月份。
...
![板块 API 配置](../assets/github/api-module-configuration.png)

这里按设计师实际使用的六个板块统一管理接口、模型、工作流和价格。正常情况下只使用第一项 `板块配置`。

### 8.1 最简单的接入方法

1. 找到要接入的板块，点击 `立即配置`。
2. 已经填过同一家接口时选择 `选择已有 API`；第一次接入时选择 `新增 API`。
3. 新增时填写 API 服务名称、协议、Base URL 和 API Key。
4. 填写模型名称与模型 ID。
5. 普通 API 的工作流留空。
6. 填写模型附加积分、人民币成本、最大并发和板块基础积分。
7. 保持 `设计师端启用` 打开，点击 `保存并同步到设计师端`。
8. 查看左侧对应板块是否已经显示实际积分。
...
- `API 服务与密钥`：填写 Base URL 和 API Key。
- `工作流（高级）`：RunningHub、ComfyUI、内部工作流才需要。
- `模型（高级）`：同一接口需要多个模型或替代模型时使用。

新手不需要依次进入这三个页面；用 `板块配置` 的弹窗即可一次完成。

### 8.3 单独维护 API 服务

操作步骤：

1. 进入 `板块 API 配置 -> API 服务与密钥`。
2. 填显示名称、调用协议、Base URL 和 API Key。
3. 打开启用，点击 `保存到服务端`。
4. 保存后页面只显示“已加密配置”，不会返回密钥原文。
...
设计师不需要手动上传或配置数据库。软件内生成的结果会在任务成功后由服务端自动写入公司素材库；本地导入或画布产生的素材也会静默同步。服务端使用稳定素材标识避免网络重试产生重复记录，并保留失败状态供运维排查和重试。

设计师只会在自己的素材库看到有权限访问的作品；管理员仍可按设计师、部门或项目监管全部公司素材。对象存储地址、账号和密钥只由运维在服务端配置，不能填写到设计师端。
...
```json
{
  ...
  "task_id": "服务端任务 UUID",
  "app_key": "只在服务端注入",
  "input_image": "不含 data:image 前缀的 Base64",
  "cut_width": 200,
  "redraw_width": 200,
  "blur_amount": 100,
  "redraw_strength": 1,
...
4. 选择小花或大花预设；需要时调整高级参数。小花默认 `100 / 100 / 50 / 0.5`，大花默认 `200 / 200 / 100 / 1`。
...
### 8.7 接入 API Mart GPT-Image-2

GPT-Image-2 是异步图像接口：提交后先返回任务 ID，系统会在后台轮询到完成，再把结果保存到公司素材库、右侧历史出图和任务历史。设计师端不会看到 API Key。

1. 用超级管理员登录 `后台管理 -> API 服务与密钥`，新建 Provider。
2. 填写 `API Mart GPT-Image-2`，协议选择 `openai`，Base URL 填 `https://api.apimart.ai/v1`。
3. 在 `API Key` 输入密钥并保存。保存后只会显示“已加密配置”，不能从网页或接口取回原文。
4. 打开 `工作流管理`，点击 `GPT-Image-2 preset`，选择刚创建的 Provider 后保存。
5. 打开 `模型 API`，新建模型：显示名称填 `GPT-Image-2`，模型 ID 填 `gpt-image-2`，绑定刚才的工作流，能力至少选择 `generate` 和 `edit`。
6. 在 `板块 API 配置` 把文生图、图片编辑或无缝拼接绑定到这个模型，并设置对应积分、人民币成本和启用状态。
7. 以设计师账号提交一张 `1k` 测试图。任务状态应依次为 `等待中 -> 处理中 -> 成功`；完成后结果会留在当前处理结果、右侧历史出图和素材库。

尺寸会自动换算为接口支持的比例。`2048x1152` 会提交为 `16:9 + 2k`，`2160x3840` 会提交为 `9:16 + 4k`。图片编辑时，上传的公司素材会由服务端转成 Base64 参考图，最多 16 张；不会暴露可访问的对象存储地址。

### 8.8 接入 APIMart Gemini 3.1 Pro 对话

![板块 API 配置](../assets/github/api-module-configuration.png)

`gemini-3.1-pro-preview` 走 Gemini 原生格式，不走 OpenAI `Responses` 格式。请由超级管理员完成下面配置：

1. 进入 `后台管理 -> 板块 API 配置 -> API 服务与密钥`。
2. 新建服务，协议选 `gemini`，Base URL 填 `https://api.apimart.ai`。
3. 仅在管理员端填写 API Key 并保存；页面只会显示“已加密配置”。
4. 新建模型，显示名称填 `Gemini 3.1 Pro`，模型 ID 填 `gemini-3.1-pro-preview`，能力勾选 `chat`，再设置积分、人民币成本和启用状态。
5. 以设计师账号打开 `GPT 对话` 或画布文本节点，在模型下拉中选择该模型后发送一条普通文本测试。

服务端实际请求为 `POST /v1beta/models/gemini-3.1-pro-preview:generateContent`，认证方式为 `Authorization: Bearer`，请求正文使用 `contents[].parts[]`。对话结果会从 `candidates[].content.parts[].text` 读取。设计师浏览器不会收到 API Key、Base URL 或 Provider 凭据。

当前 Gemini 原生快速接入适合普通文本和 data URL 图片对话。画布助手的函数工具调用仍应选择支持 Responses 工具协议的模型；若误选 Gemini，页面会给出明确错误，不会静默丢失工具操作。
...
![管理员历史筛选、分页与导出](../assets/github/admin-history-filter-pagination.png)
...
进行组合筛选。筛选条件变化后会从第一页重新查询服务端；底部分页可以选择每页 20、50、100 或 200 条，因此设计师和历史数量增加后也不需要一次加载全部记录。

### 10.2 导出历史记录

点击 `导出当前结果`，服务端会按照当前筛选条件和管理员权限生成 CSV。部门管理员只能导出本部门记录，超级管理员可以导出全公司记录；实际导出行数会自动写入审计日志。单次超过 50,000 条时，页面会提示缩小日期范围后重试。
