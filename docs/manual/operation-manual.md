...
软件内正式生成的图片会自动写入公司素材库。对于历史本地素材、从画布导入的图片或曾因网络失败而没有同步的素材：

1. 打开左侧的 `素材库`，找到目标素材。
2. 点击卡片上的 `上传公司库`；若此前失败，按钮会显示为 `重试上传`。
3. 上传完成后会显示 `已存入公司库`，素材会由服务端保存，可在本人的素材库、管理员素材监管和后续复用入口中访问。
4. 重复点击或网络重试使用同一个素材标识，不会在公司库中创建重复记录。

设计师只能查看自己有权限的素材；管理员仍可按设计师、部门或项目监管全部公司素材。对象存储地址、账号和密钥只由运维在服务端配置，不能填写到设计师端。
