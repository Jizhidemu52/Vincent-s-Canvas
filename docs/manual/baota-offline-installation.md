...
- 12 核/64 GB 机器可从 `WORKER_CONCURRENCY=8` 开始，单模型并发按供应商限制配置。企业部署保留 `OA_LOGIN_ENABLED=true`，复用公司 OA 员工身份，内部积分和角色入口关闭；仅本机试用设 OA 为 false。真实模型仍可能收费，正式入口须限制公司网络访问。
...
普通员工从“企业微信 → 公司 OA → 画布”进入，复用员工身份，不新增扫码或密码步骤。生产维护人员通过 HTTPS 访问 `/admin/login`，以独立密码改密并配置供应商和模型；管理员不能借维护身份自动进入员工画布，API Key 不在安装包中。实际 OA 与 IT 网络输入见 [接入说明](oa-workspace-integration.md)。

长请求/流式对话的两层代理仍需实测。验收至少覆盖：真实 OA A/B 员工的画布及媒体隔离，失效身份拒绝，管理接口保护，上传/生成/下载、第二项任务与联合备份；画布原图确认云端同步后关闭重开和换设备可恢复。独立聊天/创意草稿仍为员工本地数据。公司网络位置和出口 IP 尚待 IT 配置，需验证公司外网页、API、媒体与后端端口均不可访问，不能把“页面打开”当完成。
