...
OA 入口可能携带临时 token，外层公司网关和 OA 不得记录完整带 token 的 URL、查询参数或 Referer。仓库内层 Nginx 已使用无查询串日志、`Referrer-Policy: no-referrer` 与首页 `no-store`，但不能替代外层脱敏；目标服务器仍需检查实际配置、响应头和日志，详见 [生产验收](production-deployment.md#4-公司-oa-与网络验收)。
...
**当前 Windows 数据不会自动迁到新服务器或新员工。** 切换前导出重要画布并下载需要保留的媒体，核实文件可读。企业 OA 已同步画布可在同一部署由本人恢复，但更换部署仍须迁移方案；试用画布只是原浏览器本地数据。数据库与原图文件要联合备份，保留旧环境直到新环境验收完成。
...
复用公司 OA，不使用画布独立企业微信扫码，以下四项留空；实际 OA 接入见 [公司 OA 接入](oa-workspace-integration.md)：
...
根目录生产模板启用企业 OA 模式，普通用户从“企业微信 → 公司 OA → 画布”进入，不另行扫码或输入密码。`/api/deployment` 应返回 `oaLoginEnabled=true`、`authenticationEnabled=true`；内部积分和角色入口关闭：

```dotenv
OA_LOGIN_ENABLED=true
CREDITS_ENABLED=false
ROLE_PORTALS_ENABLED=false
```

仅隔离本机试用设 `OA_LOGIN_ENABLED=false`，不把它当成正式企业入口。企业模式缺失/失效身份不回退访客。公司服务器位置与出口 IP 尚待 IT 确认，网关白名单须覆盖网页、API、媒体和后端端口，不能只限制登录来源。内部积分关闭不代表供应商免费。Provider Key 在受保护的生产管理员后台录入，不写入前端或 Git；不得在升级时更换已有 `PROVIDER_ENCRYPTION_KEY`。
...
3. 公司用户从企业微信的 OA 菜单进入画布，验证员工身份后加载本人数据；不额外扫码、输密码或申请内部额度。
...
上线前确认：真实 OA 员工 A/B 分别进入且画布和媒体互不可见；失效身份拒绝、不回退访客；画布原图已同步后刷新、关闭重开和服务重启可恢复；独立管理员密码与角色保护正常，不能借管理员进入员工画布；公司外网络不能访问网页、API 或媒体；上传/生成/下载、ZIP 和第二项任务正常。独立聊天与创意草稿仍为员工本地数据，不能当全部云同步。当前真实公司环境仍待验收，不能用 `/api/health` 成功代替。
