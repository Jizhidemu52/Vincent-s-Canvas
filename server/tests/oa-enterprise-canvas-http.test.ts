...
test("cloud canvas HTTP persists whole documents with employee, revision and byte-preserving media boundaries", async () => {
  const directory = mkdtempSync(join(tmpdir(), "canvas-document-http-"));
  const statePath = join(directory, "state.sqlite");
  const identities = new DemoOaStore(statePath);
  const a = identities.login({ id: "employee-a", name: "甲" });
  ...
  const start = () => Bun.spawn([process.execPath, "src/demo-server.ts"], {
    ...
    env: { ...process.env, DEMO_STATE_PATH: statePath, DEMO_RECOVERY_FILE: "", DEMO_PORT: String(port), DEMO_HOST: "127.0.0.1", OA_LOGIN_ENABLED: "true", AUTH_ENABLED: "true", CREDITS_ENABLED: "false", LOCAL_STANDALONE: "false", STANDALONE_WEB_DIR: "", OPENTOKEN_API_KEY: "", APIMART_API_KEY: "", GPT_IMAGE_2_API_KEY: "" },
  });
  let handle = start();
  const ready = async () => {
    for (let i = 0; i < 100; i++) {
  ...
  try {
    await ready();
    expect((await fetch(base + "/api/canvas-documents")).status).toBe(401);
    ...
    expect(await request("/api/canvas-documents", b).then(r => r.json())).toEqual({ documents: [] });
    const denied = await put(b, source.id, 0, source);
    expect(denied.status).toBe(403);
    expect(await denied.json()).toMatchObject({ error: "CANVAS_ASSET_FORBIDDEN" });
    ...
    const bob = { ...source, id: "canvas-b", nodes: [], title: "乙的画布" };
    expect((await put(b, bob.id, 0, bob)).status).toBe(200);
    ...
    expect(await request("/api/canvas-documents", b).then(r => r.json())).toMatchObject({ documents: [{ id: bob.id, revision: 1, document: bob }] });
    ...
    expect((await request(assetUrl, b)).status).toBe(404);
