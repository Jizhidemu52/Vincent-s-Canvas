...
test("WeCom rejects admin starts and legacy admin callback states even with role portals hidden", async () => {
  ...
  try {
    ...
    expect((await fetch(`${origin}/auth/wecom/start?portal=admin`)).status).toBe(403);
    expect((await fetch(`${origin}/auth/wecom/callback?state=legacy-admin&code=unused`)).status).toBe(400);
  } finally {
