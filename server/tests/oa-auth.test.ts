...
test("OA deployment rejects every legacy login before database or upstream access", async () => {
  ...
  await serve(app, async (origin) => {
    for (const [path, method] of [["/login", "POST"], ["/LOGIN/", "POST"], ["/wecom/start", "GET"], ["/WECOM/start/", "GET"], ["/wecom/callback", "GET"], ["/change-password", "POST"], ["/CHANGE-PASSWORD/", "POST"]]) {
      expect((await fetch(`${origin}/auth${path}`, { method })).status).toBe(403);
    }
  });
});
