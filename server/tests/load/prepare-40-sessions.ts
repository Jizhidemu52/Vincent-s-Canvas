...
const adminInitialPassword = process.env.BOOTSTRAP_ADMIN_PASSWORD ?? "ComposeAdmin2026";
...
async function api<T>(path: string, options: RequestInit = {}, cookie?: string): Promise<ApiResult<T>> {
...
function assertStatus<T>(result: ApiResult<T>, expected: number, action: string): asserts result is ApiResult<T> & { body: T } {
    if (result.response.status !== expected) {
        throw new Error(`${action} failed (${result.response.status}): ${JSON.stringify(result.body)}`);
    }
}

function sessionCookie(response: Response) {
    const value = response.headers.get("set-cookie")?.split(";", 1)[0];
    if (!value) throw new Error("Login response did not include a session cookie");
    return value;
}
...
let adminCookie = await login("admin", adminInitialPassword, "admin");
await changePassword(adminCookie, adminInitialPassword, adminReadyPassword);

const mfaSetup = await api<{ secret: string }>("/api/auth/mfa/setup", { method: "POST" }, adminCookie);
assertStatus(mfaSetup, 200, "Administrator MFA setup");
const mfaEnable = await api("/api/auth/mfa/enable", {
    method: "POST",
    body: JSON.stringify({ code: totpCode(mfaSetup.body.secret) }),
}, adminCookie);
assertStatus(mfaEnable, 204, "Administrator MFA enablement");

const departmentResult = await api<{ department: { id: string } }>("/api/admin/departments", {
    method: "POST",
    body: JSON.stringify({ name: "40人并发验证部门", code: "load-40" }),
}, adminCookie);
assertStatus(departmentResult, 201, "Load-test department creation");

const providerResult = await api<{ provider: { id: string } }>("/api/admin/model-configuration/providers", {
    method: "POST",
    body: JSON.stringify({
...
const modelResult = await api<{ model: { id: string } }>("/api/admin/model-configuration/models", {
    method: "POST",
    body: JSON.stringify({
...
const bulkResult = await api<{ created: number; failures: unknown[] }>("/api/admin/accounts/bulk", {
    method: "POST",
    body: JSON.stringify({ accounts }),
}, adminCookie);
assertStatus(bulkResult, 201, "40-account import");
if (bulkResult.body.created !== 40 || bulkResult.body.failures.length !== 0) {
    throw new Error(`Expected 40 imported accounts: ${JSON.stringify(bulkResult.body)}`);
}

const sessionCookies = await Promise.all(accounts.map(async (account) => {
    const cookie = await login(account.username, designerInitialPassword, "designer");
    await changePassword(cookie, designerInitialPassword, designerReadyPassword);
    return cookie;
}));
...
console.log(`Prepared ${sessionCookies.length} designer sessions and one enabled mock model`);
