...
- 使用方式：企业模式由 `OA_LOGIN_ENABLED=true` 显式启用，使用者按“企业微信 → 公司 OA → 画布”进入，复用 OA 员工身份，不新增扫码或密码步骤。无效/缺失凭证不回退访客；先完成身份验证再加载员工数据。`false` 保留本机免登录试用，不能作为企业隔离环境。普通导航聚焦创作，积分与角色入口关闭，敏感模型凭据及管理接口仍受保护。

- 企业画布、节点、原图及画布内聊天使用现有员工云端文档与媒体保存，本地缓存按员工分区；独立聊天与创意草稿仍为员工分区的浏览器数据，不承诺全部云同步。试用画布本地保存。图片保留原始字节，不能只存临时 blob URL；旧账号/共用数据不自动迁移或公开，重要内容另行导出备份。
- 公司网络限制由 IT 在可信网关/防火墙配置，覆盖网页、API、媒体和后端端口；具体网络位置与出口 IP 未确认前不得猜配，不宣称限制已落实。已下载或缓存的内容不能远程收回。真实 OA 双员工与目标服务器验收状态以接入说明和待验收文档为准，健康接口成功不代表交付完成。
