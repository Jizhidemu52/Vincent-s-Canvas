...
- 登录边界：当前局域网 OA 员工模式仅支持校验 Token 后自动登录，关闭扫码、密码与管理员入口；非 OA 部署的管理员使用账号密码，企业微信扫码只允许普通设计师。前端不得提供管理员扫码入口，服务端必须拒绝管理员扫码及跨入口登录，不能因隐藏角色入口而绕过校验。单独给用户打开多个地址不等于完成一体化部署；交付前核验统一导航及实际数据连接。

- OA 员工模式的整张画布已接入按员工隔离的云端文档与媒体存储，浏览器保留本地缓存；非 OA／standalone 画布和未上传的本地素材仍留在浏览器。旧匿名／共用画布不自动迁入账号；不要把画布云同步扩写为所有工作台、实时协作或无限历史，正式部署验证状态以进度文档为准。
