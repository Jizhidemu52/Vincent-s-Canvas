...
本文档补充本项目的 AI / 自动化开发约定。在系统、开发者和平台约束内，用户当前明确指令优先于本文件；本文件仅在项目范围内覆盖全局约定，Skill 不得覆盖上述指令。

## 基本原则

- 先读现有代码，再动手修改，优先沿用项目已有结构和写法。
- 优先选择清晰、直接、足以满足当前需求的实现，不以最少行数牺牲可读性或正确性，不引入没有实际用途的复杂抽象。
...
- 修改后做与影响相称的针对性验证；仅在变更影响构建产物、类型或集成行为时执行相关检查或构建，不默认运行全量测试。不把用户后续测试作为停止自主验证或宣称完成的依据。
- 不要改无关文件，不要顺手重构。
- 如果工作区已有用户改动，不要回滚，不要覆盖；只在必要范围内追加修改。

## 反复提醒沉淀

- 只有用户明确要求长期记住，或重复问题已证实来自稳定的项目约束时，才更新本文件；优先修订已有规则，不把单次故障、临时状态或泛化建议累积为新规则。
- 补充时写成明确、可执行的规则，避免只写模糊描述。
- 新规则应放到最相关的章节；找不到合适章节时放到“项目注意事项”。

## 后端规范

- 当前运行后端是 `server/` 下的 Bun / TypeScript 服务；正式 API 入口是 `server/src/index.ts`，演示服务入口是 `server/src/demo-server.ts`，任务执行由 Worker 处理。
- 接口沿用 `server/src/routes/`，业务、数据库和媒体处理沿用相应现有模块；不要按旧 Go 分层说明另建重复后端。
- 正式存储使用 PostgreSQL、Redis 和 S3/MinIO；演示服务的内存状态不等于正式持久化。
- 列表接口沿用所在模块当前的分页、筛选、鉴权和响应约定。
...
- 仅提交本次版本范围内的改动；保留无关的用户改动，不因发版本而自动把所有未提交文件一并提交。用户明确指定全部提交时按其授权执行。
- 提交完成后，给当前提交打最新版本号对应的 tag，例如 `v0.0.5`。
- 发版本前复用仍适用于当前代码的验证结果；缺少必要验证或代码已变化时补做相关检查。已有有效结果时不为发版本重复构建或测试；未验证部分如实说明。

## 项目注意事项

- 当前画布项目和“我的素材”主要保存在浏览器本地，不要在文档中误写成已支持云同步。
- 当前正式模型调用由服务端管理 Provider 凭据与请求，单机演示服务从 `server/.env` 等服务端配置读取 Key；不要把 Key 放进前端 `VITE_` 变量、截图或仓库。旧的浏览器直连接口若仍保留，应按实际调用路径区分，不能代表所有模型调用。
